#!/usr/bin/env bash
#
# Applies/removes labels on the triggering issue. The issue number is read
# from the workflow event payload (never from an argument), so it cannot be
# redirected to another issue via prompt injection in the issue body.
#
# Usage: ./scripts/edit-issue-labels.sh --add-label bug --add-label triaged --remove-label needs-info
#
...
for label in "${ADD_LABELS[@]}"; do
  if echo "$VALID_LABELS" | grep -qxF "$label"; then
    ...
    echo "Warning: label '$label' does not exist, skipping" >&2
  fi
done
