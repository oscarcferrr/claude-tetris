...
- **Random piece selection** (`randomType`/`randomPiece`): weighted draw from `PIECE_WEIGHTS` (index = piece type, weight 0 = never drawn — `SINGLE` and the un-drawable slot 0). `makePowerPiece()` draws only from the 7 classic tetrominoes and stamps one random filled cell with a power-up code (see below).
...
- `game.js`: `COLS`, `ROWS`, `BLOCK`, `COLORS`, `PIECE_WEIGHTS`, `POWERUP_LINE_INTERVAL`, `POWERUP_CHANCE`, `FREEZE_MS`, `BOMB_RADIUS`, `POWER_SCORES`, `dropInterval` formula.
